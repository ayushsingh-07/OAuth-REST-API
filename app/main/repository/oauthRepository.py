...
class OAuthRepository:
    def _POST_(self, json: dict) -> [dict, int]:
        ...
        try:
            ...
            # update `roles_master`
            data = RolesMaster(
                role_id=_role_id_, user_id=_user_id_, role_name=json.user_role
            )
            db.session.add(data)  # add to staging area
            db.session.commit()  # permanently add data to db
    ...
    def __fetch_password_hash__(self, username: str) -> str:
        ...
        try:
            ...
            return {
                "status": "failed",
                "user": {"username": f"{username}", "password": "None"},
                "remarks": "unable to fetch data",
                "errorMsg": str(err),
            }

        SCHEMA = lambda row: {"username": row.uName, "password": row.pHash}
